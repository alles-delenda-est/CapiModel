...
**Status:** Implemented. The reference implementation is `src/simulation-engine.js` (every non-trivial line tagged `// Spec §X.Y eq (N)`); the §11.3 regression trace lives at `tests/fixtures/v1.0a-default-trace.json`. v1.0a corrections versus v1.0 are summarised in `CHANGELOG.md` at repo root.
