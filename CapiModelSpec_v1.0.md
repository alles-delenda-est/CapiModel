    # CapiModel — Technical Specification v1.0
...
- All rates that the user supplies through the UI are quoted in their conventional public-data form: `r_f` and `r_c` are **real**, `r_d` is **nominal**, `π` is the **annual inflation rate**, all other growth rates are **real** unless suffixed `_n`. Internal Fisher conversions to nominal are applied transparently in §5.1; **the user never sees a Fisher conversion in the UI.**
...
| `r_f` | 0.045 | S | OECD 60/40 portfolio real return, historical median. |
...
| `r_d_base` | 0.035 | S | Nominal. 4.0% would be the cautious projection absent any policy response. We use 3.5% on the assumption that market response to a credible reform plan would compress not just the endogenous spread (modelled separately) but also flatter the broader yield curve via reduced sovereign-risk premium embedded in OAT-bund spreads.
| `r_d_base` | 0.035 | S | Nominal. France OAT 10y is ~3.4–3.7% in early-to-mid 2026, near multi-year highs after a 37 bps rise in 2025; the 2015–2024 average of 1.4% is no longer a useful anchor in the post-2024 interest rate environment. 4.0% would be the cautious projection absent any policy response. We use 3.5% on the assumption that market response to a credible reform (especially on the scale of a serious plan to fix the pensions issue, let alone the optional labor market and demographic reform steps)plan would compress not just the endogenous spread (modelled separately) but also flatter the broader yield curve via reduced sovereign-risk premium embedded in OAT-bund spreads - absent a return to ZIRP. |
...
### 5.5 Équinoxe pension reduction

When `useEquinoxe = true`, three components apply jointly, all subject to the same time-phasing factor:

```
# Component 1: progressive bracket reduction (DREES integral)
S0_brackets = ∑_d  (R_t / 10) × (∫ r(p) × p dp / width_d) × 12 / 1e3        (18)
# where r(p) is the Équinoxe step-rate function defined in (18a),
# R_t is current retiree count = R0 × retireeIdx(t).
...
# Components 2 & 3: parameters from §3.5
S0_total      = S0_brackets + S0_irDeduction + S0_csg                        (19)
phaseFactor_t = depends on equinoxePhasing (modes below)                     (20)
S0_t          = S0_total × phaseFactor_t                                     (21)
E0_net_t      = E0 - S0_t                                                    (22)
```
...
legacyExp_t       = max(0, E0_net_t × legacyRetirees(t) × I_t)              (25)
```
...
annuityRate_t   = r_f > 0.001
                  ? r_f / (1 - (1 + r_f)^(-T_ret(t)))
                  : 1 / T_ret(t)
capiRetireeShare_t = (retireeIdx(t) > 0) ? capiRetirees(t) / retireeIdx(t) : 0
potBasedPayout_t   = K_t × annuityRate_t × capiRetireeShare_t               (53)
...
Pricing the annuity at `r_f` rather than `r_c_eff_t`: the capi pot invests at risky `r_c` during accumulation, but at retirement the retiree wants an inflation-linked annuity, which must be priced off the risk-free real rate. An insurer or state guarantor cannot promise a 4.5%-real annuity if their reserves are matched to inflation-linked sovereign bonds yielding ~3% real. Using `r_f` is standard pension-finance practice; it also has the desirable property that under the GE penalty regime (where `r_c_eff → 0`), the *pot accumulation* slows but the *annuity rate* stays stable — making payouts a direct function of pot size rather than amplifying pot stress through pricing.
...
- `r_f = r_c = 0.030`, `F0 = 220`
- `Y0 = 2026`, `lifeExpAt65_Y0 = 21.55`
- Use the v0.11 stage parameter set (cohIdx with T_extinct = 70, not 45)

This guarantees the v1.0 engine is a strict superset of v0.11 behaviour. **Note:** several v1.0 changes (45-year extinction, GDP-tracking baseline debt, F0 = 340, r_f = r_c = 0.045) are intentional model corrections rather than bugs to preserve; the backward-compat test exists to confirm the engine *can* reproduce v0.11 when explicitly forced, not as a default mode.
