...
// =====================================================================
// Counterfactual + individual perspective helpers
//
// These are pedagogical projections layered on top of the simulation
// engine. They do not change any §1–§9 spec equations; they translate
// engine outputs into per-individual euro amounts a non-specialist
// reader can interpret ("if I'm born in 1985, what does this mean?").
// =====================================================================
 ...
 *
 * @param {object} reformParams
 * @returns {object} CF parameters for runSimulation
 */
...
/**
 * Median worker pedagogical projection.
 *
 * Walks the reform simulation year-by-year for one hypothetical worker
 * born in `birthYear`, accumulates their personal capi pot (if eligible
 * by cutoffAge), and computes monthly euro pension amounts at
 * retirement. Compares against the no-reform counterfactual.
 *
 * Hard-coded median-worker assumptions:
 *  - 30 M actives → individual share of W0 = W0 / 30 (≈k€/yr/worker)
 *  - working window: age 22 → retirementAgeBase
 *  - life expectancy: 85 (annuity tenor = 85 − retirementAgeBase)
 *  - capi pot grows at the engine's effective nominal capi rate (r_cn_eff_t),
 *    which already includes the GE penalty
 *
 * @param {object} cfg          reform parameter set
 * @param {Array}  reformRows   runSimulation(cfg)
 * @param {Array}  cfRows       runSimulation(buildCounterfactualParams(cfg))
 * @param {number} birthYear    1965–2010 (UI clamps further)
 */
export function computeIndividualPerspective(cfg, reformRows, cfRows, birthYear) {
  ...
  const inCapi = cfg.enableCapi
    && (cfg.cutoffAge == null || ageInY0 < cfg.cutoffAge);
  ...
  // Walk the reform sim, accumulating personal capi pot.
  let capiPot = 0;
  let capiPotAtRet = 0;
  ...
  // Average legacy pension at retirement year. legacyRetirees is a
  // multiplier (1.0 = R0 baseline), so retirees in millions = mult × R0.
  // Md€/yr ÷ M retirees = k€/yr/retiree → /12 = k€/month → ×1000 = €/month.
  const avgLegacyPensionMonthly = (rows, t) => {
    const r = rows[Math.min(t, rows.length - 1)];
    const retireesM = Math.max((r.legacyRetirees ?? 0) * cfg.R0, 1e-9);
    return (r.legacyExp_t / retireesM) * KE_TO_EUR / 12;
  };

  const monthlyPensionCF = avgLegacyPensionMonthly(cfRows, retT);
  const avgLegacyPensionReform = avgLegacyPensionMonthly(reformRows, retT);

  // Capi annuity from personal pot. Annuity factor uses real return
  // (r_c) over (LIFE_EXPECTANCY − RETIREMENT_AGE) years.
  ...
  // Dual-rights model for transitional cohorts:
  //   yearsInPayg = years the worker spent in PAYG before the capi cutoff
  //   yearsInCapi = years in capi up to retirement
  //   careerYears = retirementAge − 22 (full career window)
  // Legacy pension is prorated by yearsInPayg / careerYears, giving
  // transitional workers a partial PAYG pension for accrued rights plus
  // their capi annuity. Pre-cutoff (older) cohorts get the full average
  // legacy pension; post-Y0 cohorts (born after the transition) get
  // capi only. This is the pedagogical view that matches the v0.11
  // dual-rights panel and produces a monotonically rising total income
  // by birth year. It does NOT match the engine's binary aggregate
  // (where legacyExp_t excludes capi-cohort accruals); the panel is a
  // per-worker projection, not an aggregate sum.
  const careerYears = Math.max(1, RETIREMENT_AGE - 22);
  const yearsInPayg = inCapi
    ? Math.max(0, Math.min(ageInY0, RETIREMENT_AGE) - 22)
    : careerYears;
  const legacyShare = yearsInPayg / careerYears;
  const monthlyPensionLegacy = avgLegacyPensionReform * legacyShare;
